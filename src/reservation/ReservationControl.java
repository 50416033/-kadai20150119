...
public String deleteReservation(MainFrame frame){
	...
	if ( flagLogin){ // ログインしていた場合
		...
		// 予約のキャンセル画面の予約日に，メイン画面に設定されている年月日を設定する
		rd.tfYear.setText(frame.tfYear.getText());
		rd.tfMonth.setText(frame.tfMonth.getText());
		rd.tfDay.setText(frame.tfDay.getText());
...
public String reservationConfirm(){
	...
	Frame frame = null;
		//(1) MySQL を使用する準備
		ReservationConfirm fd = new ReservationConfirm(frame);
		fd.setVisible(true);
		fd.setModalityType(LoginDialog.ModalityType.APPLICATION_MODAL);
		//IDとパスワードの入力がキャンセルされたら,空文字列を結果として終了
		if ( fd.canceled){
			return "";
		}
		...
		//ユーザID入力された場合の処理
		String userid = fd.tfUserID.getText();
	...
	try {
		// 予約情報を取得するクエリ
		ResultSet rs = mysql.getdate(userid);
		boolean exist = false;
		while(rs.next()){
...
public String facilitySummary( String facility){
	String res = "";
	...
	try {
		// 予約情報を取得するクエリ
		ResultSet rs = mysql.facilityReservation(facility);
		boolean exist = false;
		while(rs.next()){
